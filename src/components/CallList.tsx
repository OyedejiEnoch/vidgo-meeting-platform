  ...
  return (
    <div className='grid grid-cols-1 gap-5 xl:grid-cols-2'>
      {calls && calls.length > 0 ? calls.map((meeting:Call |CallRecording)=>(
        ...
        title={(meeting as Call).state?.custom?.description?.substring(0, 20) || meeting?.filename?.substring(0, 20) 
            || "Personal Meeting"}
         mainIcon={
            type === 'ended'
            ? '/icons/previous.svg'
         ...
         date={meeting.state?.startsAt.toLocaleString() || meeting.startTime.toLocaleString()} 
         firstButtonText={type === "recording" ? "Play" : "Start"}
         link={type === 'recording' ? meeting.url : `${process.env.NEXT_PUBLIC_BASE_URL}/meeting/${meeting.id}`}
        secondButtonText={type === "recording" ? "Share" : "Copy Link"}
         secondButtonIcon='' 
